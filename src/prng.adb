...
package body prng is
    ...
    begin
        ...
        Internal_State.Screen.Print((Componant_Line(display.Entropy), 0), "Entropy :=" & Actual_Entropy_Count'Img & "/" & Max_Pool_Entropy'Img & "        ", Send_USART => False);
    ...
    begin
        ...
        while Nb_Bits_Work > 0 loop
            while get_entropy < Min_Entropy and entropy_pool.remaining_extracted = 0 loop
                delay 0.1;
                Internal_State.Screen.Print((Componant_Line(display.Log_Info), 0), "Wait. for more entr.", Send_USART => False);
            end loop;
            Internal_State.Screen.Print((Componant_Line(display.Log_Info), 0), "Creat. random number", Send_USART => False);
        ...
        Internal_State.Screen.Print((Componant_Line(display.Log_Info), 0), "Done: " & Interfaces.C.Strings.Value(Buffer) & "          ", Send_USART => False);
    end Random_Internal;
